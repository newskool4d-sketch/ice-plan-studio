...
export function ThumbnailRail({ projection, page, onSelectPage, issueCountByPage = {} }) {
...
function RenderedPreview({ realPreview, zoom }) {
  return <div className="paper-wrap composition-svg-wrap" style={{ transform: `scale(${zoom / 75})` }}>
    <img className="composition-svg" src={`data:image/svg+xml;charset=utf-8,${encodeURIComponent(realPreview.svg)}`} alt={`생성 HWPX 실조판 ${realPreview.pageCount}쪽`} />
  </div>;
}
...
export function DocumentStage({
  ...
  return <section className="document-stage">
    ...
    </div><span>{current ? `${current.number}쪽 · ${current.label}` : "입력 대기"}</span></div>
    ...
    <div className="canvas-scroll">
      {previewMode === "compare" && realPreview && hasQuick ? <div className="compare-grid">
        ...
        <figure className="compare-pane"><figcaption>실조판 SVG · 전체 {realPreview.pageCount}쪽</figcaption><RenderedPreview realPreview={realPreview} zoom={compareZoom} /></figure>
      </div>
        : previewMode === "rendered" && realPreview ? <RenderedPreview realPreview={realPreview} zoom={zoom} />
