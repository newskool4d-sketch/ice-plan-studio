...
// 미리보기 투영 블록의 rows는 머리글 제외 데이터 행이다(모델 계약). HWPX 물리 행과
// 비교할 때만 [header, ...rows] 전체 그리드로 조립한다.
const gridOf = (block) => [block.header || [], ...(block.rows || [])];
const expectedTables = preview.pages.flatMap((page) => page.blocks).filter((block) => block.type === "table" && !isTocTable(gridOf(block)));
...
for (const [index, expected] of expectedTables.entries()) {
  ...
  if (!actual) continue;
  const expectedGrid = gridOf(expected);
  if (actual.widthHwpUnit !== expected.widthHwpUnit) failures.push(`표 ${index + 1} 너비: ${actual.widthHwpUnit} != ${expected.widthHwpUnit}`);
  if (actual.rowCnt !== expectedGrid.length || actual.colCnt !== expectedGrid[0].length) failures.push(`표 ${index + 1} 구조: ${actual.rowCnt}x${actual.colCnt} != ${expectedGrid.length}x${expectedGrid[0].length}`);
  if (actual.repeatHeader !== preview.tokens.table.repeatHeader) failures.push(`표 ${index + 1} 머리글 반복 불일치`);
  if (JSON.stringify(actual.rows) !== JSON.stringify(expectedGrid)) failures.push(`표 ${index + 1} 셀 내용 불일치`);
}
