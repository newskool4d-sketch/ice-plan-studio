...
기준일: 2026-07-19 (한글 실물 검증 + 실전 샘플 검증 + 렌더링 크래시 수정 + CI·슬로건 표지 페이지 + 배포성 확보 + 프로필·프로젝트 저장/복원 실클릭 검증 반영)
...
## 프로필·프로젝트 저장·복원 실클릭 검증 (2026-07-19)

CDP로 검증 상태를 준비(실전 샘플 로드 → 교육지원청 프로필 전환 → 커스텀 CI 등록)한 뒤,
computer-use 실클릭으로 저장·복원 두 사이클을 전부 검증.

### 프로필 저장·복원

1. "프로필 저장" 클릭 → 네이티브 저장 다이얼로그(파일명 "기관-브랜딩" 자동 채움) →
   `verify-profile.iceprofile` 저장 → "프로필을 저장했습니다" 확인
2. CDP로 프로필을 본청으로 되돌려 상태 오염
3. "프로필 불러오기" 클릭 → 저장한 파일 선택 → "프로필을 불러왔습니다" 확인,
   기관명 "교육지원청" 복원, 커스텀 CI 로고 복원(화면 육안 확인)

### 프로젝트 저장 → **완전 재시작(cold restart)** → 복원

1. "프로젝트 저장" 클릭 → `verify-project.iceplan` 저장 → 파일 내용 직접 검사:
   `agencyId: "district"`, `loadedFile`, 96블록 `loadedModel`, `customBranding.district.ci`
   (base64) 전부 포함 확인
2. **`Get-Process | Stop-Process`로 앱을 완전히 종료**(프로세스 0개 확인) →
   새 프로세스로 처음부터 재시작 → 초기 상태(본청·데모 화면) 확인
3. "프로젝트 불러오기" 클릭 → 저장 파일 선택 → "프로젝트를 불러왔습니다" 확인
4. CDP로 최종 상태 검증: `profile: "district"`, 커스텀 CI base64 실제 로드
   (`naturalWidth: 5906`, `complete: true`), 문서 정보 카드("96개 블록 · 규칙 0건")
   정확, "HWPX 내보내기" 버튼 활성화, **콘솔 예외 0건**

**결론: 프로필·프로젝트 저장/복원이 앱을 완전히 새로 시작한 뒤에도 문서·기관·커스텀
브랜딩까지 손실 없이 복원됨을 실클릭으로 확인.**

## 남은 수동 QA

없음 — 이번 세션에서 파악·요청된 항목은 전부 해소됨. `ICE Plan Studio Setup 0.1.0.exe`는
문서 미리보기·검토 패널 연결, CI·슬로건 표지 페이지 임베드, 타 PC 배포성 확보까지 전부
반영해 재빌드 완료했고, CDP로 콘솔 예외 0건을 재확인했다. 프로필·프로젝트 저장/복원도
cold-restart 시나리오까지 실클릭으로 검증 완료.
